...
def get_institution_members(self,
    ...
    """Получить информацио о ученных из института"""
    ...
    if self._page_404():
        raise PageNotFoundException  # если нет, завершить выкинуть исключение
    ...
    while this_page_is_valid:
        ...
        if self._page_404() or current_page == stop_page + 1:
            stop_page = current_page - 1
            this_page_is_valid = False

    # Запись в
    with open(f'{self._data_dir_path}/members/{institution.replace("-", "_")}_{start_page}-{stop_page}.json', 'w',
              encoding='utf-8') as js_out:
        json.dump(institution_members, js_out)
